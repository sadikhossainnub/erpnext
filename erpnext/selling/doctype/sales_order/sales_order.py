...
class SalesOrder(SellingController):
	...
	def validate_stock_availability(self):
		if not frappe.get_single_value("Selling Settings", "validate_stock_availability_for_sales_order"):
			return

		reqd_qty_map = {}
		for d in self.get("items"):
			if cint(frappe.get_cached_value("Item", d.item_code, "is_stock_item")):
				key = (d.item_code, d.warehouse)
				reqd_qty_map.setdefault(key, 0)
				reqd_qty_map[key] += flt(d.stock_qty)

		for (item_code, warehouse), required_qty in reqd_qty_map.items():
			actual_qty, reserved_qty = frappe.db.get_value(
				"Bin",
			...
			if available_qty < required_qty:
				frappe.throw(
					_("Cannot submit Sales Order. Insufficient available stock in warehouse {0} for item {1}. Available to reserve: {2}, Required: {3}").format(
						frappe.bold(warehouse), frappe.bold(item_code), available_qty, required_qty
					)
				)
