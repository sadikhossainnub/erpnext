erpnext.PointOfSale.ItemCart = class {
			...
			`<div class="cart-container">
				<div class="abs-cart-container">
					...
					<div class="cart-header">
						...
						<div class="discount-header">${__("Discount")}</div>
	...
	make_cart_totals_section() {
		...
		this.$totals_section.append(
			...
			<div class="loyalty-points-container"></div>
			<div class="grand-total-container">
				<div>${__("Grand Total")}</div>
				<div>0.00</div>
			</div>
			<div class="checkout-btn">${__("Checkout")}</div>
			<div class="coupon-btn">${__("Coupon")}</div>
			<div class="gift-card-btn">${__("Gift Card")}</div>
	...
	bind_events() {
		...
		this.$component.on("click", ".coupon-btn", () => {
			this.show_coupon_control();
		});

		this.$component.on("click", ".redeem-btn", () => {
			this.show_loyalty_redemption_dialog();
		});

		this.$component.on("click", ".gift-card-btn", () => {
			this.show_gift_card_control();
		});
	...
	show_coupon_control() {
		const me = this;
		const frm = me.events.get_frm();
		let coupon_code = frm.doc.coupon_code || "";

		const dialog = new frappe.ui.Dialog({
			title: __("Apply Coupon Code"),
			fields: [
				{
			...
			primary_action(values) {
				frappe.model.set_value(frm.doc.doctype, frm.doc.name, "coupon_code", values.coupon_code);
				dialog.hide();
			},
		});
	...
	show_gift_card_control() {
		const me = this;
		const frm = me.events.get_frm();
		const current_code = frm.doc.gift_card_code || "";

		const dialog = new frappe.ui.Dialog({
			title: __("Redeem Gift Card"),
			fields: [
				{
				...
				},
				{
					...
					fieldtype: "HTML",
					label: __("Gift Card Info"),
				},
				{
					fieldname: "redemption_amount",
					fieldtype: "Currency",
					label: __("Redemption Amount"),
					hidden: 1,
				},
			],
			...
			primary_action(values) {
				frappe.call({
					method: "erpnext.accounts.doctype.gift_card.gift_card.validate_gift_card",
					args: { code: values.gift_card_code },
					callback: function (r) {
						if (r.message) {
							const gc = r.message;
							const grand_total = flt(frm.doc.rounded_total || frm.doc.grand_total);
							const max_redeem = Math.min(gc.balance, grand_total);

							dialog.fields_dict.gift_card_info.$wrapper.html(
								`<div class="gift-card-info" style="padding: 10px; background: var(--bg-green); border-radius: 8px; margin: 8px 0;">
									<div style="font-weight: 600; color: var(--text-color);">${__("Gift Card Balance")}: ${format_currency(gc.balance)}</div>
									${gc.expiry_date ? '<div style="color: var(--text-muted); font-size: 12px;">' + __("Expires") + ': ' + gc.expiry_date + '</div>' : ''}
								</div>`
							);

							const redemption_field = dialog.fields_dict.redemption_amount;
							redemption_field.df.hidden = 0;
							redemption_field.df.description = __("Max redeemable: {0}", [format_currency(max_redeem)]);
							redemption_field.set_value(max_redeem);
							redemption_field.refresh();

							dialog.set_primary_action(__("Redeem"), async function () {
								const amount = flt(dialog.get_value("redemption_amount"));
								if (amount <= 0) {
									frappe.msgprint(__("Please enter a valid redemption amount."));
									return;
								}
								if (amount > gc.balance) {
									frappe.msgprint(__("Amount exceeds gift card balance."));
									return;
								}
								if (amount > grand_total) {
									frappe.msgprint(__("Amount exceeds grand total."));
									return;
								}

								await frappe.model.set_value(frm.doc.doctype, frm.doc.name, "gift_card_code", values.gift_card_code);
								await frappe.model.set_value(frm.doc.doctype, frm.doc.name, "gift_card_redemption_amount", amount);

								me.update_totals_section(frm);
								dialog.hide();
								frappe.show_alert({
									message: __("Gift Card {0} applied: {1}", [values.gift_card_code, format_currency(amount)]),
									indicator: "green",
								});
							});
						}
		...
		if (current_code) {
			dialog.$wrapper.find(".modal-footer").prepend(
				`<button class="btn btn-danger btn-sm btn-remove-gc" style="position: absolute; left: 15px;">${__("Remove Gift Card")}</button>`
			);
			dialog.$wrapper.on("click", ".btn-remove-gc", async () => {
				await frappe.model.set_value(frm.doc.doctype, frm.doc.name, "gift_card_code", "");
				await frappe.model.set_value(frm.doc.doctype, frm.doc.name, "gift_card_redemption_amount", 0);
				me.update_totals_section(frm);
				dialog.hide();
				frappe.show_alert({ message: __("Gift Card removed."), indicator: "orange" });
			});
		}
	...
	show_loyalty_redemption_dialog() {
		const me = this;
		const frm = me.events.get_frm();
		const { loyalty_points, conversion_factor } = this.customer_info;
		const max_redeemable_amount = flt(loyalty_points * conversion_factor);
			...
			fields: [
				{
					...
					default: frm.doc.loyalty_amount || 0,
				},
			],
			primary_action_label: __("Redeem"),
			async primary_action(values) {
				const amount = flt(values.loyalty_amount);
				if (amount > max_redeemable_amount) {
					frappe.msgprint(__("Cannot redeem more than {0}", [format_currency(max_redeemable_amount)]));
					return;
				}

				await frappe.model.set_value(frm.doc.doctype, frm.doc.name, "redeem_loyalty_points", amount > 0 ? 1 : 0);
				await frappe.model.set_value(frm.doc.doctype, frm.doc.name, "loyalty_amount", amount);
	...
	update_customer_section() {
		...
		function get_customer_description() {
			...
			if (!email_id && !mobile_no) {
				desc = `<div class="customer-desc">${__("Click to add email / phone")}</div>`;
			} else if (email_id && !mobile_no) {
				desc = `<div class="customer-desc">${email_id}</div>`;
			} else if (mobile_no && !email_id) {
				desc = `<div class="customer-desc">${mobile_no}</div>`;
			} else {
				desc = `<div class="customer-desc">${email_id} - ${mobile_no}</div>`;
			}

			if (loyalty_points) {
				desc += `<div class="customer-desc" style="color: var(--blue-500); font-weight: 500;">⭐ ${loyalty_points} Points</div>`;
			}
	...
	update_totals_section(frm) {
		...
		this.update_loyalty_points_section();
		this.update_gift_card_section(frm);
	}

	update_gift_card_section(frm) {
		if (!frm) frm = this.events.get_frm();
		const gc_code = frm.doc.gift_card_code;
		const gc_amount = flt(frm.doc.gift_card_redemption_amount);
		let $gc_container = this.$totals_section.find(".gift-card-container");

		if (!$gc_container.length) {
			this.$totals_section.append('<div class="gift-card-container" style="display:none;"></div>');
			$gc_container = this.$totals_section.find(".gift-card-container");
		}

		if (gc_code && gc_amount > 0) {
			const currency = frm.doc.currency;
			$gc_container.css("display", "flex").html(
				`<div style="display:flex;justify-content:space-between;align-items:center;width:100%;padding:8px 12px;background:var(--bg-green);border-radius:8px;margin-top:6px;">
					<div style="font-weight:600; color:var(--text-color);">🎁 ${__("Gift Card")} (${gc_code})</div>
					<div style="font-weight:600; color:var(--dark-green-500);">-${format_currency(gc_amount, currency)}</div>
				</div>`
			);
		} else {
			$gc_container.css("display", "none").html("");
		}
	}

	update_loyalty_points_section() {
		const { loyalty_points } = this.customer_info || {};
		if (loyalty_points) {
			this.$totals_section
				.find(".loyalty-points-container")
				.css("display", "flex")
				.html(`
					<div class="loyalty-label">
						⭐ ${__("Loyalty Points")}: <span class="loyalty-value">${loyalty_points}</span>
					</div>
					<div class="redeem-btn">${__("Redeem")}</div>
				`);
		} else {
			this.$totals_section.find(".loyalty-points-container").css("display", "none").html("");
		}
	}
	...
	render_cart_item(item_data, $item_to_update) {
		...
		function get_rate_discount_html() {
			const discount_html = `<div class="item-discount" style="width: 25%; text-align: center;">${format_currency(item_data.discount_amount || 0, currency)}</div>`;
	...
	highlight_checkout_btn(toggle) {
		if (toggle) {
			...
			this.allow_coupon_code && this.$cart_container.find(".coupon-btn").css("display", "flex");
			this.allow_gift_card && this.$cart_container.find(".gift-card-btn").css("display", "flex");
		} else {
			...
			this.$cart_container.find(".coupon-btn").css("display", "none");
			this.$cart_container.find(".gift-card-btn").css("display", "none");
		}
	}
