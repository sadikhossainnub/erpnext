...
erpnext.PointOfSale.Payment = class {
	...
	bind_coupon_code_event(frm) {
		if (frm.doc.coupon_code && !frm.applying_pos_coupon_code) {
			if (!this.settings.allow_coupon_code) {
				frappe.show_alert({
					message: __("Coupon Code is disabled in this POS Profile."),
					indicator: "orange",
				});
	...
	render_loyalty_points_payment_mode() {
		...
		if (!this.settings.allow_loyalty_point_redemption) return;
	...
	update_totals_section(doc) {
		...
		const grand_total = cint(frappe.sys_defaults.disable_rounded_total)
			? doc.grand_total
			: doc.rounded_total;
		const gc_amount = flt(doc.gift_card_redemption_amount);
		const effective_total = grand_total - gc_amount;
		const remaining = effective_total - doc.paid_amount;
		const change = doc.change_amount || remaining <= 0 ? -1 * remaining : undefined;
		const currency = doc.currency;
		const label = doc.paid_amount > effective_total ? __("Change Amount") : __("Remaining Amount");
		...
		let gc_html = "";
		if (gc_amount > 0 && doc.gift_card_code) {
			gc_html = `
			<div class="seperator-y"></div>
			<div class="col">
				<div class="total-label">🎁 ${__("Gift Card")}</div>
				<div class="value text-success">-${format_currency(gc_amount, currency)}</div>
			</div>`;
		}
		...
		this.$totals.html(
			...
			<div class="col">
				...
				<div class="value ${doc.paid_amount < effective_total ? "text-danger" : "text-success"}">${format_currency(
				change || remaining,
				currency
			)}</div>
			</div>`
