...
class SalesInvoice(SellingController):
	...
	def on_submit(self):
		...
		# Redeem gift card balance on submit
		if self.gift_card_code and flt(self.gift_card_redemption_amount) > 0:
			from erpnext.accounts.doctype.gift_card.gift_card import redeem_gift_card

			redeem_gift_card(self.gift_card_code, self.gift_card_redemption_amount, self.name)
	...
	def on_cancel(self):
		...
		# Restore gift card balance on cancel
		if self.gift_card_code and flt(self.gift_card_redemption_amount) > 0:
			from erpnext.accounts.doctype.gift_card.gift_card import restore_gift_card_balance

			restore_gift_card_balance(self.gift_card_code, self.gift_card_redemption_amount)
	...
	def make_loyalty_point_entry(self):
		...
		loyalty_program = frappe.get_doc("Loyalty Program", self.loyalty_program)
		...
		eligible_amount = flt(self.grand_total) - flt(self.loyalty_amount) - self.get_returned_amount()
		...
		# Helper function to find a tier by name
		...
		# State Machine
		current_tier_name = customer.get("loyalty_tier")
		current_tier = get_tier(current_tier_name)

		# Ensure cumulative amount is ALWAYS tracked
		customer.cumulative_purchase_amount = flt(customer.get("cumulative_purchase_amount", 0.0)) + flt(self.grand_total)

		if not current_tier_name or not current_tier:
			# No Tier Logic - Check if now eligible for first tier
			first_tier = tiers[0]
			if customer.cumulative_purchase_amount >= flt(first_tier.get("min_spent", 0.0)):
				# Upgrade to first tier
				customer.loyalty_tier = first_tier.tier_name
				customer.tier_start_date = today()
				customer.tier_expiry_date = add_days(today(), cint(first_tier.get("tier_duration_days", 365)))
				points_earned = cint(eligible_amount / (flt(first_tier.get("collection_factor")) or 1.0))
			else:
				# Remains No Tier
				points_earned = 0
		else:
			# Has an active Tier
			expiry_date = getdate(customer.tier_expiry_date)
			tier_index = get_tier_index(current_tier_name)
			
			if expiry_date < getdate(today()):
				# Tier Expired -> Downgrade
				if tier_index == 0:
					# Downgrade to No Tier
					customer.loyalty_tier = None
					customer.tier_start_date = None
					customer.tier_expiry_date = None
					customer.cumulative_purchase_amount = 0.0
				else:
					# Downgrade to previous tier
					prev_tier = tiers[tier_index - 1]
					customer.loyalty_tier = prev_tier.tier_name
					customer.tier_start_date = today()
					customer.tier_expiry_date = add_days(today(), cint(prev_tier.get("tier_duration_days", 365)))
					customer.cumulative_purchase_amount = 0.0
				
				points_earned = 0 # No points upon expiry/downgrade evaluation
			else:
				# Active Tier, Not Expired
				collection_factor = flt(current_tier.get("collection_factor")) or 1.0
				points_earned = cint(eligible_amount / collection_factor)
				
				# (Cumulative already increased above)
				
				# Check Upgrade
				upgrade_target = flt(current_tier.get("upgrade_target_amount"))
				if upgrade_target > 0 and customer.cumulative_purchase_amount >= upgrade_target:
					if tier_index + 1 < len(tiers):
						next_tier = tiers[tier_index + 1]
						customer.loyalty_tier = next_tier.tier_name
						customer.tier_start_date = today()
						customer.tier_expiry_date = add_days(today(), cint(next_tier.get("tier_duration_days", 365)))
						# REMOVED: customer.cumulative_purchase_amount = 0.0 # Reset upon upgrade
						# Points are still given normally on upgrade as currently defined
		
		# Save customer state
		customer.flags.ignore_permissions = True
		customer.save()

		# Generate Point Entry if points > 0
		if points_earned > 0:
			doc = frappe.get_doc(
				{
					...
					"loyalty_program_tier": customer.loyalty_tier,
					"customer": self.customer,
					"invoice_type": self.doctype,
					"invoice": self.name,
					"loyalty_points": points_earned,
					"purchase_amount": eligible_amount,
					"expiry_date": add_days(self.posting_date, loyalty_program.expiry_duration),
			...
			# Ensure sales invoice's tier is updated for historical reference
			self.db_set("loyalty_program_tier", customer.loyalty_tier)
